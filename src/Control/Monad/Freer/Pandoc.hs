...
writeTo :: FR.Member FromPandoc effs => PandocWriteFormat a -> P.WriterOptions -> P.Pandoc -> FR.Eff effs a
...
toPandoc prf pro x = read pro x where
  read = case prf of
    ReadDocX -> P.readDocx
...
fromPandoc :: P.PandocMonad m => PandocWriteFormat a -> P.WriterOptions -> P.Pandoc -> m a
fromPandoc pwf pwo pdoc = write pwo pdoc where
  write = case pwf of
...
newPandoc :: (FR.PandocEffects effs, FR.Member Pandocs effs) => T.Text -> FR.Eff (ToPandoc ': effs) () -> FR.Eff effs ()
newPandoc n l = (fmap snd $ FR.runWriter $ toWriter l) >>= newPandocPure n 

namedPandocFrom :: P.PandocMonad m => PandocWriteFormat a -> P.WriterOptions -> NamedDoc (P.Pandoc) -> m (NamedDoc a)
namedPandocFrom pwf pwo (NamedDoc n pdoc) = do
  doc <- fromPandoc pwf pwo pdoc
  return $ NamedDoc n doc

pandocsToNamed :: FR.PandocEffects effs => PandocWriteFormat a -> P.WriterOptions -> FR.Eff (Pandocs ': effs) () -> FR.Eff effs [NamedDoc a]
pandocsToNamed pwf pwo = join . fmap (traverse (namedPandocFrom pwf pwo)) . toNamedDocList -- monad, list, NamedDoc itself
               
fromPandocE :: FR.PandocEffects effs => PandocWriteFormat a -> P.WriterOptions -> FR.Eff (ToPandoc ': effs) () -> FR.Eff effs a
fromPandocE pwf pwo = join . fmap (fromPandoc pwf pwo . snd) . FR.runWriter . toWriter
