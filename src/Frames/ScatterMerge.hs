  ...
  let process :: [(V.Snd x, V.Snd w)] -> F.Record '[x,w] -> [(V.Snd x, V.Snd w)]
      ...
      extract rt' l =
        let compFst (x,_) (x',_) = compare x x'
            ...
            lowerBounds x bs = case nonEmpty x of
              Nothing -> bs
              Just neX ->
                let nextLB = (\(a,_,_) -> a) . head $ neX
                    newX = List.dropWhile (\(_,_,sw) -> realToFrac sw < weightPerBin * fromIntegral (List.length bs + 1)) x
                    newBS = bs ++ [nextLB]
                in lowerBounds newX newBS
        in uncurry (BinsWithRescale (lowerBounds listWithSummedWeights [])) scaleInfo
...
sortedListToBinLookup' as a = let xs = maybe [] tail $ nonEmpty as in 1 + fromMaybe (List.length xs) (List.findIndex (>a) xs)
...
newtype Bin2DT = Bin2D (Int, Int) deriving (Show, Eq, Ord)
...
scatterMerge' toX toY xBins yBins =
  let binningInfo :: Real c => BinsWithRescale c -> (c -> Int, c -> Double)
      ...
      extractF = F.concat <$> MR.mapReduceFold MR.noUnpack (MR.splitOnKeys @'[Bin2D])
